...
### Changed
- The documentation guide is split by reader need: `concepts.md` covers the show-dispatch model, the Pluto/non-Pluto routing and `CONTEXT`; `guide.md` keeps task-oriented recipes with headings naming the problem each solves; `documenter.md` covers the `disable_html_show!` integration. The guide keeps its filename and anchor, so existing `/stable/guide/` links still resolve.
